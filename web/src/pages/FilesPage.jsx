            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">
              ...
              {data.files.map((m) => (
                <div
                  ...
                  className={`group overflow-hidden rounded-xl border bg-ink-850/70 transition ${
                ...
                >
                  ...
                  <div className="p-2">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1 opacity-0 transition group-hover:opacity-100">
                      <Mini onClick={() => setClip({ mode: 'cut', type: 'media', id: m.id, name: m.title || m.filename })}>Cut</Mini>
                      <Mini onClick={() => setClip({ mode: 'copy', type: 'media', id: m.id, name: m.title || m.filename })}>Copy</Mini>
                      <Mini onClick={() => nav(`/watch/${m.id}`)}>Play</Mini>
                      {canDelete && <Mini danger onClick={() => delFile(m)}>Delete</Mini>}
                    </div>
                  </div>
                </div>
              ))}
